...
export default class extends Controller {
  ...
  updateSimulationSummary() {
    ...
    const productSummaries = Array.from(selectedProducts).map(checkbox => {
      ...
      const profitMatch = profitText.match(/Rp\s?([\d,]+)\s+profit\/unit/)
    ...
    summaryHTML += `
      <div class="pt-2 border-t border-gray-200 space-y-1">
        ...
        <div class="flex justify-between text-sm">
          ...
          <span class="font-bold text-green-600">Rp ${totalRevenue.toLocaleString('id-ID')}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-900 font-medium">Potential Profit:</span>
          <span class="font-bold text-green-600">Rp ${totalProfit.toLocaleString('id-ID')}</span>
        </div>
      </div>
    </div>`
